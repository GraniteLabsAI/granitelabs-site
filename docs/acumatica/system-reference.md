---
title: Acumatica System Reference
description: Overview of Acumatica ERP system architecture, workflows, and financial management capabilities.
image: /img/og/acumatica-og.jpg
---

# Acumatica – System Reference

![Acumatica](/img/og/acumatica-og.jpg)

## 🔹 Overview
This page provides a structured reference of key areas within Acumatica.

---

## 🎯 Objective
- Provide quick lookup for system areas
- Support troubleshooting and reporting
- Organize ERP knowledge

---

## 🧠 Key Areas

### Projects (PM)
- Job cost tracking
- Budgets vs actuals
- Cost categories

---

### Financials
- General ledger
- Accounts payable
- Accounts receivable

---

### Reporting
- Velixo integration
- Generic inquiries
- Financial reports

---

### Structure
- Companies vs branches
- Shared vs separate data
- Intercompany considerations

---

## 📊 Related Pages
- [Overview](/acumatica/overview/)

---

## ✅ Result
A structured reference for navigating and understanding Acumatica system components.
